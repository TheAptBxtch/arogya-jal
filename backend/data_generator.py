...
def generate_synthetic_pump_data():
    ...
    while current_time < end_time and len(data) < 1000:
        ...
        if random.random() < 0.018:  # ~1.8% chance per reading
...
def main():
    ...
    print(f"Failure events: {df['failure_in_72_hours'].sum()} ({df['failure_in_72_hours'].sum()/len(df)*100:.1f}%)")
